...
class Gaussian:
    ...
    def absolute_square(self):
        """
        create Gaussian which is the |G|^2 of the original Gaussian G
        """
        A2 = self.A + self.A.conjugate()
        B2 = self.B + self.B.conjugate()
        return Gaussian(A2,B2)
    ...
    def sample_prob(self, nr_initial_conditions):
        """
        draw samples from the probability distribution |Psi|^2(q) = G(q)
        Unlike sampling from the Wigner distribution, this gives us
        only position but no momenta.
        ...
        ========
        qs: Samples 
          qs[:,n] are the initial positions of the n-th trajectory
        """
        # covariance matrix
        cov = la.inv(self.A)
        # mean
...
#        random.seed(int(time.time()*1000))
        ...
        dim = N
        qs = samples
        # qs[d,:] -> initial positions on d-axis
        return qs
    def sample_wigner(self, nr_initial_conditions, pw=1.0):
        """
        Parameters:
        ===========
        nr_initial_conditions: number of samples that should be drawn
...
#        random.seed(int(time.time()*1000))
        ...
        ps = samples[dim:,:]
        # qs[d,:] -> initial positions on d-axis
        # ps[d,:] -> initial momenta along d-axis for all samples
        return (qs, ps)
