...
class ThomsonProblem:
    def __init__(self, n1, n2, Ncap=0, a=4.6487, L=20.0, north_south="N"): #18.897): #4*18.897):
        """
        Parameters:
        ===========
        n1, n2: chiral indeces of the nanotube
        Ncap: number of Thomson points making up the cap, if set to 0
          a number is derived from the density of carbon atoms in graphene
    ...
    def addCap(self):
        chiral_vector, tube_vector, a1, a2, c1, c2 = chiral_tube_vectors(self.n1,self.n2, self.L, self.a)
        # Determine how many Thomson points should be placed on the hemisphere
        # area of unit cell
        Aunit = la.norm(np.cross(a1,a2))
        # area of hemisphere
        circ = la.norm(chiral_vector) # circumferance
        R = circ/(2.0*np.pi) # radius of sphere
        # area of hemisphere
        Ahemi = 0.5 * 4.0 * np.pi * pow(R,2) 
        ...
        #
        self.R = R
        x_tube, o_tube, s_tube = thomson_tube(chiral_vector, tube_vector, a1, a2)
        ...
        if self.north_south == "N":
            ...
            self.x = reflect_tube_xy(self.x, self.s, self.L)

        atomlist = self.thomson2atomlist()
        XYZ.write_xyz("/tmp/thomson_opt.xyz", [atomlist], mode="w")

        active, inactive = active_layers(self.x,self.s,a1,a2)

        def savexyz():
            atomlist = self.thomson2atomlist()
            XYZ.write_xyz("/tmp/thomson_opt.xyz", [atomlist], mode="a")

        self.x[active,:], self.s[active], self.o[active], self.en, self.grad = optimize_thomson(self.x[active,:], self.s[active], self.o[active], R=self.R, callback=savexyz)
    ...
    def dual_lattice(self):
        ...
        if self.north_south == "S":
            atomlist = reflect_atomlist_xy(atomlist, self.L)
        return atomlist
    def triangulate(self):
        # for each vertex determine the atoms that are closer than 
        # (1+eps)*(lattice constant a)
        r2,dr2d1,dr2d2 = thomson.thomson.rij2(self.x,self.s,self.o, self.R, self.Z0, 0)
...
def active_layers(x,s,a1,a2):
    ...
    dz_layer = 2*la.norm(a1+a2) # approximate width of one layer
    n,dummy = x.shape
    for i in range(n):
        if s[i] == "T":
            z,phi = x[i,:]
...
def find_best_cap(n1,n2,L, Ncap=0, Ntrial=10, north_south="N"):
    """
...
def capped_nanotube(n1,n2,L,NcapN=0, NcapS=0):
    ...
    # north pole
    atomlistN = find_best_cap(n1,n2,L,Ncap=NcapN,Ntrial=100, north_south="N")
    # south pole
    atomlistS = find_best_cap(n1,n2,L,Ncap=NcapS,Ntrial=100, north_south="S")
