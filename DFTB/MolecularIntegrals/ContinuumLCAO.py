...
from DFTB.MolecularIntegrals.BasissetFreeDFT import BasissetFreeDFT, density_func, effective_potential_func, residual_func, residual_ee_func, orbital_transformation, energy_correction, inhomogeneous_schroedinger, add_two_functions, laplacian_func, regular_coulomb_func
...
def variational_mixture_continuum(atomlist, phi, delta_phi, potential, E,
                                  rsmall=0.05, rlarge=10.0):
    """
...
def improve_continuum_orbital(atomlist, phi0, potential, E,
    ...
    for i in range(0, max_iter):
        ...
        a,b = variational_mixture_continuum(atomlist, phi, delta_phi, potential, E)
...
def test_hmi_continuum():
    ...
    phi = improve_continuum_orbital(atomlist_grid, phi0, potential, E, thresh=1.0e-6)
...
def test_lcao_continuum():
    ...
    for lmax in [0,1,2,3]:
        ...
        a,b = variational_mixture_continuum(atomlist, phi0, delta_phi, veff, E)
        ...
        plt.plot(r, residual(x,y,z), label=r"$(H-E)(a \phi_0 + b \Delta \phi)$")
        plt.plot(r, a*residual_0(x,y,z) + b*residual_d(x,y,z), ls="-.", label=r"$(H-E)(a \phi_0 + b \Delta \phi)$ (separate)")
            
        plt.legend()
        plt.show()
        
        averaged_angular_distribution(atomlist, bound_orbitals, continuum_orbitals, E)
        ...
        for i,phi in enumerate(continuum_orbitals):
            def func(grid, dV):
                x,y,z = grid
                return phi(x,y,z)
            Cube.function_to_cubefile(atomlist, func, filename="/tmp/cmo_lmax_%2.2d_orb%4.4d.cube" % (lmax,i), ppb=5.0)
        #
        ...
        for i,phi in enumerate(continuum_orbitals):
            ...
            l, = plt.plot(r, phi(x,y,z), label=r"$\phi_{%d}$ ($l_{max}$ = %d)" % (i, lmax))
            plt.plot(r, residual(x,y,z), ls="-.", label=r"$(H-E)\phi_{%d}$" % i, color=l.get_color())
